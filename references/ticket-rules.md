...
- One ticket per `states.jsonl` row with verdict `missing` or `broken` at `medium` or above, through its `obs-` row: a missing loading, empty, or error state; a hover-only affordance with no keyboard path; a disabled control with no reason; an overlay that does not trap or return focus; a form that submits while invalid; a component with no dark partner; overflow at a declared breakpoint. Rows of the same class and state across surfaces (every button lacks focus-visible) share one ticket listing every location.
- One ticket per finding at `medium` or above, floors included: a systemic contrast or target-size cluster is one ticket listing every location, never dropped because it was reported once. Findings with the same root cause (the same rule, component, or markup duplicated across files) share one ticket listing every finding id and every file.
...
- `confirmed` claims never produce tickets; their `obs-` rows do. A ticket's `sources` list holds only contradicted claims, failed expectations, observations, and findings; a confirmed claim that gives context is quoted in the body, never listed as a source.
- A page-level aggregate row (`keyboard-only`, `narrow:<w>`, `dark`) whose causes are ticketed separately gets no ticket of its own; the systemic ticket for the shared cause lists the page rows under `related`.
- Never bundle unrelated contradictions. Traceability beats tidiness.

## Type and severity

`type`: `functional | visual | ux | a11y | copy | console-error | performance`. A ticket with several sources takes the type of its first-listed source, except state-gap tickets (any source is a `state-gap` or `transformation` observation, a `states`/`transformation` finding, or a `looks`/`state` claim about a component state), which take their type from the gap: focus, keyboard, trap, contrast, name -> `a11y`; menu, dialog, empty state, feedback, timing, hover-only affordance -> `ux`; a skeleton that never resolves, a raw error on screen, a form that submits wrong -> `functional`; a light island or overflow -> `visual`. A `promise` claim whose capability is missing is `functional` even when the copy is therefore also wrong. Claims of kind `does`, `flows`, `state` -> `functional`; `promise` -> `functional` when the capability is missing, `copy` when the statement is wrong; `exists` contradicted -> `functional` when the element is a control, `visual` when decorative; `shows` -> `copy`; `looks` -> `visual`. Observations and findings keep the category they were given: navigation, states, functional-affordance -> `ux` or `functional`; contrast, hit-areas, accessibility -> `a11y`.
